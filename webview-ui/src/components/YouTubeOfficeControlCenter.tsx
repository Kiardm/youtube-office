...
type YouTubeConnection = { configured: boolean; connected: boolean; channelId?: string | null; channelTitle?: string | null; autoPublishEnabled: boolean; connectedAt?: string | null; lastCheckedAt?: string | null; blocker?: string | null; finishedRoot: string }
...
export function YouTubeOfficeControlCenter({ state, initialSection = 'Office', onClose }: { state: { agents?: Record<string, { name: string; role: string; personality: string; quirks?: string[] | string }>; timeline?: Array<{ id: string; timestamp: string; type: string; reason?: string; status?: string | null }>; activeProject?: { title: string } | null; coop?: { mode: string; activeRoomId?: string | null }; workday?: { reflections?: unknown[] }; deliveryManifests?: Delivery[]; publishJobs?: PublishJob[] }; initialSection?: Section; onClose(): void }) {
  ...
  const connectYouTube = async () => {
    try {
      await post('/publishing/youtube/connect', { clientId: youtubeClientId })
  ...
  return <div className="yt-office-control-center" role="dialog" aria-label="YouTube Office 4.2.1 control center">
    <header><div><strong>YouTube Office 4.2.1</strong><span>Local control center</span></div><button onClick={onClose}>Close</button></header>
