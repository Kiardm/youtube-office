...
type Section = 'Office' | 'Projects' | 'Conversation History' | 'Worker Personalities' | 'Memory Center' | 'Providers' | 'Publishing' | 'Permissions' | 'Co-op Rooms' | 'Activity Log' | 'Backups and Updates'
...
type YouTubeConnection = { configured: boolean; clientSecretConfigured: boolean; connected: boolean; channelId?: string | null; channelTitle?: string | null; autoPublishEnabled: boolean; connectedAt?: string | null; lastCheckedAt?: string | null; blocker?: string | null; finishedRoot: string }
type Delivery = { projectId: string; projectTitle?: string; finalPath: string; sha256: string; bytes: number; deliveredAt: string; status: string }
type PublishJob = { id: string; projectId: string; status: string; visibility?: string; videoId?: string | null; publicUrl?: string | null; blocker?: string | null; percent?: number; createdAt: string }
...
export function YouTubeOfficeControlCenter({ state, initialSection = 'Office', onClose }: { state: { agents?: Record<string, { name: string; role: string; personality: string; quirks?: string[] | string }>; timeline?: Array<{ id: string; timestamp: string; type: string; reason?: string; status?: string | null }>; activeProject?: { title: string } | null; coop?: { mode: string; activeRoomId?: string | null }; workday?: { reflections?: unknown[] }; deliveryManifests?: Delivery[]; publishJobs?: PublishJob[] }; initialSection?: Section; onClose(): void }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [p, m, c, b, room, reports, youtubeStatus] = await Promise.all([get('/providers'), get('/memory'), get('/capabilities'), get('/backups'), get('/coop/status'), get('/usage-reports'), get('/publishing/youtube')])
      setProviders(p); setMemories(m.records || []); setCapabilities(c); setBackups(b.backups || []); setCoop(room); setUsageReports(reports.reports || []); setYoutube(youtubeStatus)
  ...
  const connectYouTube = async () => {
    try {
      await post('/publishing/youtube/connect', { clientId: youtubeClientId, clientSecret: youtubeClientSecret })
      setNotice('Google authorization opened. Return here after selecting the YouTube channel account.')
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Could not start YouTube connection.') }
  }

  return <div className="yt-office-control-center" role="dialog" aria-label="YouTube Office 4.2.2 control center">
    <header><div><strong>YouTube Office 4.2.2</strong><span>Local control center</span></div><button onClick={onClose}>Close</button></header>
    ...
    <main>
      ...
      {section === 'Projects' && <section><h2>Projects</h2><p>{state.activeProject ? `Active: ${state.activeProject.title}` : 'No project is active. Workers are idle.'}</p><p>Every approved project is delivered to a clean folder containing only its current finished product.</p>{(state.deliveryManifests || []).slice().reverse().map((delivery) => { const job = (state.publishJobs || []).filter((item) => item.projectId === delivery.projectId).at(-1); return <article key={`${delivery.projectId}-${delivery.sha256}`}><h3>{delivery.projectTitle || delivery.projectId}</h3><p>{delivery.finalPath}</p><small>SHA-256 {delivery.sha256} · {Math.round(delivery.bytes / 1024 / 1024)} MiB · {job ? `YouTube: ${job.status}` : 'Not uploaded'}</small>{job?.publicUrl && <p><a href={job.publicUrl} target="_blank" rel="noreferrer">Open YouTube video</a></p>}<button onClick={() => action(() => post(`/projects/${encodeURIComponent(delivery.projectId)}/open-finished`), 'Opened the finished product in File Explorer.')}>Open finished product</button><button onClick={() => navigator.clipboard.writeText(delivery.finalPath)}>Copy path</button>{job?.blocker && <p role="alert">{job.blocker}</p>}</article> })}{(state.deliveryManifests || []).length === 0 && <p>No finished products have been delivered yet. Candidate reports remain internal until Manager approval is conclusive.</p>}</section>}
      ...
      {section === 'Publishing' && <section><h2>Publishing</h2><p>Manager-approved videos are staged privately, verified after YouTube processing, and only then made public. Credentials remain encrypted on this computer.</p><article><h3>YouTube channel</h3>{youtube?.connected ? <><p>Connected: <strong>{youtube.channelTitle}</strong> ({youtube.channelId})</p><small>Finished products: {youtube.finishedRoot}</small><label>Automatic publishing<input type="checkbox" checked={youtube.autoPublishEnabled} onChange={(event) => action(() => post('/publishing/youtube/policy', { autoPublishEnabled: event.target.checked }), event.target.checked ? 'Automatic Manager publishing enabled.' : 'Automatic publishing disabled.')} /></label><button onClick={() => action(() => post('/publishing/youtube/disconnect'), 'YouTube credentials removed from this computer.')}>Disconnect YouTube</button></> : <><p>Not connected. Enter the YouTube Data API desktop client ID and secret for the one-time sign-in. Both stay local; the secret is Windows-encrypted.</p><label>OAuth desktop client ID<input value={youtubeClientId} onChange={(event) => setYoutubeClientId(event.target.value)} placeholder="123…apps.googleusercontent.com" /></label><label>OAuth desktop client secret<input type="password" value={youtubeClientSecret} onChange={(event) => setYoutubeClientSecret(event.target.value)} placeholder={youtube?.clientSecretConfigured ? 'Encrypted secret already stored' : 'GOCSPX-…'} /></label><button disabled={!youtubeClientId.trim() || (!youtubeClientSecret.trim() && !youtube?.clientSecretConfigured)} onClick={() => void connectYouTube()}>Connect YouTube</button></>}{youtube?.blocker && <p role="alert">{youtube.blocker}</p>}<button onClick={() => void refresh()}>Refresh connection</button></article><h3>Recent publishing jobs</h3>{(state.publishJobs || []).slice(-10).reverse().map((job) => <article key={job.id}><h3>{job.projectId} · {job.status}</h3><p>{job.videoId ? `Video ${job.videoId}` : `${job.percent || 0}% uploaded`}</p>{job.publicUrl && <a href={job.publicUrl} target="_blank" rel="noreferrer">{job.publicUrl}</a>}{job.blocker && <p role="alert">{job.blocker}</p>}{job.blocker && job.status !== 'public' && <button onClick={() => action(() => post(`/publishing/jobs/${encodeURIComponent(job.id)}/retry`), 'Publishing retry completed.')}>Retry publishing</button>}</article>)}</section>}
